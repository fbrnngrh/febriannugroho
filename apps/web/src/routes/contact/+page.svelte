<script lang="ts">
  ...
  import { Copy, Check, Send } from "lucide-svelte";
...
<div class="space-y-10 reveal">
  ...
  <div class="space-y-4">
    <div>
      ...
      <div class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm text-muted">
        ...
        <span class="hidden sm:inline text-neutral-300 dark:text-neutral-700 font-normal"
          >&bull;</span
        >
        <div class="flex items-center gap-1.5">
    ...
    <p class="text-base text-muted/95 leading-relaxed max-w-lg">
      Have a freelance project, a full-time role opportunity, or just want to connect? Hit the
      composer below or email me directly at:
    </p>
    ...
    <div class="inline-flex items-center gap-2 group/email text-lg sm:text-xl font-medium">
      ...
      >
        {#if copiedEmail}
          <Check class="h-4.5 w-4.5 text-green-500 pop-in" />
        {:else}
  ...
  <div class="space-y-6 pt-2">
    <div class="space-y-1">
      ...
      <p class="text-xs text-muted">Fill this form to draft a structured email immediately.</p>
    </div>

    <form onsubmit={handleCompose} class="space-y-6">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div class="space-y-1 input-focus-line">
          <label for="name" class="text-[10px] font-bold tracking-wider uppercase text-muted"
            >Your Name</label
          >
          <input
            type="text"
        ...
        <div class="space-y-1 input-focus-line">
          <label for="email" class="text-[10px] font-bold tracking-wider uppercase text-muted"
            >Your Email</label
          >
          <input
            type="email"
      ...
      <div class="space-y-1 input-focus-line">
        <label for="subject" class="text-[10px] font-bold tracking-wider uppercase text-muted"
          >Subject / Project Type</label
        >
        <div class="relative">
          <select
            id="subject"
      ...
      <div class="space-y-1 input-focus-line">
        <label for="message" class="text-[10px] font-bold tracking-wider uppercase text-muted"
          >Your Message</label
        >
        <textarea
          id="message"
      ...
      <div class="space-y-3 pt-2">
        <button
          ...
          class="press-feedback w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-fg hover:bg-fg/90 disabled:bg-fg/50 text-bg px-6 py-3.5 text-xs font-semibold tracking-wider uppercase transition-all duration-200 cursor-pointer disabled:cursor-not-allowed"
        >
          {#if formStatus === "opening"}
            <div
              class="h-3.5 w-3.5 border-2 border-bg border-t-transparent rounded-full animate-spin"
            ></div>
        ...
        {#if formStatus === "done"}
          <p class="fade-slide-up text-[11px] text-green-600 dark:text-green-400 font-medium">
